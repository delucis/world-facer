...
module.exports = async function paperScraper ({ n = 10, recurse = false, maxRecurse = Infinity } = {}) {
  ...
  this.recurseCount = this.recurseCount || 0 // How deep is the recursion
  ...
  console.log(`      ${this.quickmarks.length} bookmarks left to retrieve.
      Fetching full text for batch of ${n} bookmarks...`)

  const { bookmarks, failedIDs } = GET_TEXT(this.quickmarks, n, this)
  ...
  if (bookmarks) await WRITE_BOOKMARKS(bookmarks, dataDir)
...
async function GET_TEXT (quickmarks, n, ctx) {
  ...
  for (const b of quickmarks.slice(-1 * n)) {
    ...
    try {
      bookmarks[id].text = await ctx.client.bookmarks.getText(id)
      bookmarks[id].cacheTime = Date.now()
      delete failedIDs[id]
      doneids.push(id)
