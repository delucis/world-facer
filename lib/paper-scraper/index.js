...
module.exports = async function paperScraper ({ n = 10, recurse = false, maxRecurse = Infinity } = {}) {
  ...
  this.progressCache = this.quickmarks.length // Progress made last time
  ...
  if (this.quickmarks.length === 0) {
    C.l('No more bookmarks to retrieve, returning...')
    return { bookmarks: this.bookmarks, failures: this.failedIDs }
  }

  C.l(`${this.quickmarks.length} bookmarks left to retrieve.`)

  if (this.recurseCount > 1) {
    const timeRemaining = GET_TIME_REMAINING(this, { recurse, n, maxRecurse })
    C.l(`Estimated time remaining: ${timeRemaining}`)
  }

  C.l(`Fetching full text for batch of ${n} bookmarks...`)
  ...
  for (const id in failedIDs) {
    delete this.bookmarks[id]
    const i = this.quickmarks.findIndex(b => b.bookmark_id.toString() === id)
...
function GET_TIME_REMAINING (
  ...
  return MS(remaining * timeElapsed / completed, { secondsDecimalDigits: 0, compact: true })
}
...
async function FILTER_BOOKMARKS (bookmarks, dir) {
  ...
  C.l(`Discarded ${discards} bookmark${discards === 1 ? '' : 's'} that had already been retrieved`)
...
async function GET_TEXT (quickmarks, n, ctx) {
  ...
  for (const b of quickmarks.slice(-1 * n)) {
    ...
    try {
      ...
      if (!text) throw new E('NoText', 'No text returned by API')
      if (text.length < 500) throw new E('TextTooShort', 'Text returned is too short')
