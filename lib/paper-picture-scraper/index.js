...
const USER_AGENT = require('user-agents')
const UA = new USER_AGENT({ deviceCategory: 'desktop' })
...
/**
 * Execute full paper-picture-scraper scraping loop:
 ...
 * @param {Boolean} [o.recurse]     Should paperPictureScraper recurse?
 * @param {Number}  [o.maxRecurse]  How many recursions are allowed?
 */
module.exports = async function paperPictureScraper ({ batchSize, recurse, maxRecurse } = {}) {
  VLDT('N', [batchSize])
  C.l()
  C.log(C.b('Starting paper-picture-scraper image scraper'))
...
const GET_PAPER_IMG_URLS = async () => {
  ...
  const urls = (await WORDS.data()).reduce((urls, bookmark) => {
    ...
    imgs.forEach(img => {
      img.src = IS_ABSOLUTE(img.src) ? img.src : new URL(img.src, bookmark.url).href
 ...
 * @param  {Object}  meta   Metdata object for image at url
 * @param  {String}  ppsDir  Directory paperPictureScraper saves to
 * @param  {Object}  gauge  Gauge instance to update on data progress
 * @return {Promise}        Resolves once image has finished writing
 */
const DOWNLOAD_IMAGE = async (url, meta, ppsDir, gauge) => {
  VLDT('SOSO', [url, meta, ppsDir, gauge])
    ...
    get.head(url, (e, r) => {
      ...
      get(url, e => {
        ...
        .on('end', async () => {
          ...
          const imgPath = PATH.join(ppsDir, imgDir)
          const imgName = `img.${type.ext}`
          meta.filename = imgName
          meta.type = type.ext
          await MKDIR(imgPath)
          await FS.writeFile(PATH.join(imgPath, imgName), data)
            .catch(e => reject(e))
          await FS.writeFile(
            PATH.join(imgPath, 'img.json'),
            JSON.stringify(meta)
          )
            .catch(e => reject(e))
          resolve()
 ...
 * @param {Boolean}  [o.recurse=false]       Should paperPictureScraper recurse?
...
async function DOWNLOAD_IMAGES (
  ...
  this.ppsDir = this.ppsDir || PATH.resolve(__dirname, '../../data/paper-pictures')
  ...
  await THROTTLED_LOOP(L.map.toArr(batch).map(([url, meta]) => async () => {
    try {
      await DOWNLOAD_IMAGE(url, meta, this.ppsDir, this.gauge)
