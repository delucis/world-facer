...
module.exports = async function sketch ({ width, height, duration, fps }) {
  ...
  console.log(`Starting sequence on ${start}`)
  ...
  for (let i = 0; i < frames; i++) {
    ...
    const vOffset = lerpPoints(vOffsetPoints, playhead, { smooth: true })
  ...
  return async (
    { context: ctx, time, frame, fps, playhead, Image }
  ) => {
    ...
    const vOffsetFrame = CLAMP(Math.floor(frame), 0, vOffsets.length)
    const horizonOffset = vOffsets[vOffsetFrame]

    ctx.globalAlpha = barFlicker
    img1.onload = drawHorizon(ctx, img1, { aperture: heightFlicker, vOffset: horizonOffset })
    img1.src = image1
    ctx.globalAlpha = EASE(time / transitionTime % 1) * barFlicker
    const img2 = new Image()
    img2.onload = drawHorizon(ctx, img2, { aperture: heightFlicker, vOffset: horizonOffset })
    img2.src = image2

    const wordFrame = CLAMP(Math.floor(frame), 0, wordObjects.length)

    for (let idx = 0; idx < wordObjects[wordFrame].length; idx++) {
      const { text, x, y, z, size, vOffset } = wordObjects[wordFrame][idx]
      const flicker = getFlicker()
      ctx.globalAlpha = flicker * z
      drawText(ctx, text, x, y + vOffset, { baseline: 'middle', color: 'white', size: size * z * (flicker + 1) / 2 })
    }
  }
}
