...
module.exports = ({ width, height }) => {
  ...
  const gradientH = 0.25 * frameHeight
  const gradientBox = new Box([0, height - gradientH, width, gradientH])
  const gradientPoints = [1, Math.round(gradientBox.y), 1, height]
  ...
  return ({ context: ctx, time }) => {
    ...
    const bottomL = lerpPoints([[6, 50], [25, 39], [35, 43], [72, 0]], time, { easing: 'sineInOut' })
    ...
    gradient.addColorStop(1, `hsl(${bottomHue}, 100%, ${bottomL}%)`)
