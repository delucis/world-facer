...
module.exports = async function sketch ({ width, height, duration }) {
  ...
  return async (
    ...
    if (aperture < 1) {
      ...
      const upperBlurBox = new Box([0, frameTop, width, Math.max(0, horizonCenter - frameTop - aperture / 2)])
      const lowerBlurBox = new Box([0, horizonCenter + aperture / 2, width, height - horizonCenter - aperture / 2])
      ...
      brightnessRect(ctx, 0, horizonCenter - aperture / 2, width, aperture, brighten)
    }
  }
}
