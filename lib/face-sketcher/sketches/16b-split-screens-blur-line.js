...
module.exports = async function sketch ({ width, height, duration }) {
  ...
  const aperturePoints = [
    ...
    [57, 40], [66, 40],
    [81, 729]
  ].map(([x, y]) => [x, y / 1080 * height + frameTop])
  ...
  return async (
    ...
    const blurTop = lerpPoints(aperturePoints, time, { easing: 'quadIn' })
    const blurBox = new Box([0, blurTop, width, height - blurTop])

    const saturation = lerpPoints(saturationPoints, time)
    const darken = lerpPoints(darkenPoints, time)

    brightnessRect(ctx, ...blurBox, darken)
    saturateRect(ctx, ...blurBox, saturation)
    blurRect(ctx, ...blurBox, 30 / 540 * height)
  }
}
