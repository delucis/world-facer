...
module.exports = async function sketch () {
  ...
  return async (
    ...
    drawRect(ctx, 0, 0, width, height, { background: colors[colorIndex] })
    ...
    drawRect(ctx, 0, 0, width, height, { background: 'white' })
    ...
    const zoom = LERP(11, 1, EASEOUT(playhead))
    ...
    for (let i = 0; i < splits; i++) {
      ...
      const img = new Image()
      img.onload = ImageDrawer(ctx, img, i * splitWidth, 0, { h: height, w: splitWidth, zoom })
      img.src = image
    }

    // ctx.globalCompositeOperation = 'source-over'
    ctx.globalAlpha = blankSlateAlpha
    drawRect(ctx, 0, 0, width, height, { background: pink })
