...
async function makeNews () {
  ...
  const fragment = PATH.join('/audio', `${FORMAT(new Date(), 'YYYYMMDDHHmmss')}.mp4`)
  const path = PATH.join(__dirname, '../news-server', fragment)
  await MKDIR(PATH.dirname(path))
  await SAY(voiceString, { path, voice: 'Fiona', compressed: true })
  return fragment
}
