...
function getLines (context, { text = '', measure = 400 } = {}) {
  ...
  let p = words.reduce((p, word) => {
    let m = context.measureText(p.nextLine ? `${p.nextLine} ${word}` : word)
    if (m.width > measure) {
...
module.exports = function drawParagraph (
  ctx, text, x, y,
  {
    style, variant, weight, stretch, baseline,
    size = 67.5,
  ...
  for (let i = 0; i < lines.length && (i + 1) * leading < maxHeight; i++) {
