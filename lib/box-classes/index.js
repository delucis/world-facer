...
class Box extends Size {
  ...
  intersect (box) {
    ...
    const w = x2 - x1
    const h = y2 - y1
    const doIntersect = w >= 0 && h >= 0
    this.x = doIntersect ? x1 : 0
    this.y = doIntersect ? y1 : 0
    this.width = doIntersect ? w : 0
    this.height = doIntersect ? h : 0
    return this
  }

  intersected (box) { return this.clone().intersect(box) }
