...
async function LOAD_DATA (tracker) {
  ...
  const queue = new QUEUE({ concurrency: 40 })
  return queue.addAll(dirs.map(
    dir => async () => {
      ...
      data.thumb = async function () {
        const thumbPath = PATH.join(dataDir, dir, 'thumb.png')
        const cachekey = dir + '-thumb'
        if (!this[cachekey] || this[cachekey].hasExpired()) {
          const buffer = await READ(thumbPath)
          if (this[cachekey]) {
            this[cachekey].update(buffer)
          } else {
            this[cachekey] = CACHE.newCache(buffer, { maxAge: Infinity })
          }
        }
